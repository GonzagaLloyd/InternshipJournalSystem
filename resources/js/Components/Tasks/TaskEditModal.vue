...
const updateTask = () => {
    // Logic Guide: Send a patch/put request to route('tasks.update', props.task.id)
                    ...
                    <div class="relative z-10 p-8 md:p-14 overflow-y-auto max-h-[90vh] scrollbar-hide">
                        ...
                        <div class="text-center mb-10 md:mb-12 relative">
                            ...
                            <div class="flex items-center justify-center gap-4">
                                ...
                                <p class="text-[9px] uppercase tracking-[0.5em] text-[#8C6A4A] font-black font-serif opacity-80">Amending the Ledger</p>
                                <div class="h-[1px] w-12 bg-gradient-to-l from-transparent to-[#8C6A4A]/40"></div>
                            </div>
                        </div>
                        
                        <form @submit.prevent="updateTask" class="space-y-8 md:space-y-10">
                            <!-- Task Name -->
                            <div class="space-y-4">
                                ...
                                <div class="relative group">
                                    <textarea 
                                        v-model="form.name"
                                        rows="2"
                                        class="w-full bg-black/30 border border-white/5 focus:border-[#8C6A4A]/60 rounded-2xl px-6 md:px-8 py-4 md:py-5 text-[#C9B79C] focus:ring-0 transition-all font-serif text-base md:text-xl scrollbar-hide resize-none leading-relaxed"
                                        required
                                    ></textarea>
                                </div>
                            </div>

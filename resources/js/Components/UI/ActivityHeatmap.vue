...
const heatmapData = computed(() => {
    ...
    for (let i = 0; i < 53; i++) {
        ...
        for (let j = 0; j < 7; j++) {
            ...
            const count = props.activity ? (props.activity[dateStr] || 0) : 0;
...
<template>
    <div class="space-y-4">
        ...
        <div class="relative bg-black/20 rounded-sm p-4 border border-white/5 overflow-hidden">
            <template v-if="props.activity === null">
                <div class="flex flex-col gap-4">
                    <div class="flex gap-1 overflow-hidden">
                        <SkeletonLoader v-for="i in 53" :key="i" width="10px" height="88px" borderRadius="1.5px" />
                    </div>
                    <div class="flex justify-between px-1">
                        <SkeletonLoader v-for="i in 12" :key="i" width="20px" height="8px" />
                    </div>
                </div>
            </template>

...
# 7. Install Composer dependencies (scripts needed for package:discover)
COPY --from=composer:latest /usr/bin/composer /usr/bin/composer
ENV COMPOSER_MEMORY_LIMIT=-1
RUN composer install --no-dev --optimize-autoloader --ignore-platform-reqs

# 8. Ensure storage directories exist
...
# 9. Create a placeholder .env so artisan commands don't fail
RUN touch .env

# 10. Build Frontend Assets then remove Node.js to save runtime memory
...
# 12. Configure PHP for Low Memory (512MB Render Limit)
...
# 13. Limit Apache to minimal workers to save RAM
RUN echo "StartServers 1" > /etc/apache2/conf-available/mpm-tuning.conf \
    ...
    && echo "MaxRequestWorkers 10" >> /etc/apache2/conf-available/mpm-tuning.conf \
    && echo "ServerLimit 10" >> /etc/apache2/conf-available/mpm-tuning.conf \
    && a2enconf mpm-tuning
